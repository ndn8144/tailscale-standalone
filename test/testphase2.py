    ...
    for source_name, get_key_func in sources:
        try:
            ...
            if auth_key and auth_key.startswith('tskey-auth-'):
                print(f"✅ Auth key found: {source_name}")
                print(f"   Preview: {auth_key[:30]}...")
                return auth_key, source_name
            elif auth_key:
                print(f"❌ Invalid auth key format in {source_name}")
        except Exception as e:
            print(f"❌ Error reading {source_name}: {e}")
    
    print("❌ No valid auth key found")
    print("💡 Create one of these:")
...
def test_msi_download():
    ...
    print("\n3️⃣ Testing Tailscale MSI download...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ MSI download accessible: {size:.2f} MB")
            return True
        else:
            print(f"❌ MSI download failed: {response.status_code}")
            return False
            
    except Exception as e:
        print(f"❌ MSI download test failed: {e}")
        return False

def test_build_environment():
    """Test build environment"""
    print("\n4️⃣ Testing build environment...")
    ...
    try:
        ...
        if free_space < 1:
            ...
            print(f"✅ Disk space: {free_space:.2f} GB")
    except Exception:
...
def test_simple_build():
    ...
    print("\n5️⃣ Testing simple build process...")
...
def main():
    ...
    if all(results):
        ...
        print("\n5️⃣ Skipping build test due to previous failures")
    ...
    if all(results):
        print("\n🎉 ALL TESTS PASSED!")
        print("🚀 Ready to build ATT Tailscale installer!")
